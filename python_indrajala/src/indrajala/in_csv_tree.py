    ...
    def __init__(self, name, toml_data):
        ...
        try:
            if self.toml_data[self.name]["active"] is False:
                return
            self.rootpath = self.toml_data[self.name]["rootpath"].replace(
                "{configdir}", self.toml_data["indrajala"]["config_dir"]
            )
            if os.path.isdir(self.rootpath) is False:
                self.log.error(
                    f"{self.name}: Root directory {self.rootpath} does not exist"
                )
                return
        except Exception as e:
        ...
        if "time" in msg:
            if "topic" in msg:
                ...
                if msg["topic"] not in self.ev_store or date != self.current_date:
                    ...
                    if (
                        msg["topic"] in self.ev_store
                        and len(self.ev_store[msg["topic"]]["data"]) > 0
                    ):
                ...
                self.ev_store[msg["topic"]]["data"].append(
                    (msg["time"], msg["uuid"], msg["msg"])
                )
                if self.flush_data(
                    msg["topic"],
                    self.ev_store[msg["topic"]]["date"],
