        ...
        for lib_entry in self.library_state:
            ...
            if 'docs' in lib_entry:
                for doc in lib_entry['docs']:
                    fn = doc['ref_name']
                    md5_hash = hashlib.md5(fn.encode('utf-8')).hexdigest()  # kosync on filename
                    self.md5_to_lib_entry[md5_hash] = {'filename': fn, 'uuid': uuid}
                    if 'koreader_md5' in doc:
                        md5_hash = doc['koreader_md5']  # koreader on partial md5 of content (generated by ebook_tools, using koreader algorithm)
                        self.md5_to_lib_entry[md5_hash] = {'filename': fn, 'uuid': uuid}
            else:
